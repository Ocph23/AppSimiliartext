...
{
    public class StemmingProcces
    {
        public IQueryable<Data.RootWord> RootWordCollection { get; private set; }

        public StemmingProcces()
        {
            using (var db = new OcphDbContext())
            {
                this.RootWordCollection = db.RoadWords.Select();
            }
        } 
        ...
        private bool cekKataDasar(string kata)
        {
            ...
            var data = this.RootWordCollection.Where(O => O.Word == kata);
            if (data.Count() >0)
